...
./my-app --config gpu-worker.toml
```

Kubernetes can scale deployments per capability using separate node pools or taints/tolerations with different `forge.toml` configs mounted via ConfigMaps.
