...
When you remove a deprecated workflow version's code and redeploy, the new binary won't have a handler for any in-flight runs of that version. The runtime detects this at boot, logs a warning per stranded `(name, version)` group, and flips `/_api/ready` to `503` with `drain_pending > 0`. Boot still succeeds; the node stays up but the load balancer rotates traffic away until the drain count returns to zero.
