...
|-----------|------|---------|-------------|
| `discovery` | `string` | `"postgres"` | Node discovery method |
| `heartbeat_interval_secs` | `u64` | `5` | Heartbeat frequency |
| `dead_threshold_secs` | `u64` | `15` | Time until node marked dead |
| `seed_nodes` | `[string]` | `[]` | Static node addresses |
| `dns_name` | `string` | none | DNS name for discovery |

## Discovery Methods

### PostgreSQL (Default)

Nodes register in `forge_nodes` table. No additional infrastructure.
...
Resolve node addresses from DNS A/AAAA records.

```toml
[cluster]
discovery = "dns"
dns_name = "forge.service.consul"
```

Works with Consul, AWS Cloud Map, or any DNS-based service discovery.

### Kubernetes

Uses Kubernetes API to discover pods in the same namespace.
...
Reads the `KUBERNETES_SERVICE_HOST` environment variable. Requires appropriate RBAC permissions.

### Static

Hardcoded list of seed nodes.
...
For fixed infrastructure where node addresses do not change.
...
`/_dashboard` shows:
- Active nodes with status, load, roles
- Leader assignments
- Dead nodes pending cleanup

### Metrics

```
forge_cluster_nodes_total{status="active"} 5
forge_cluster_nodes_total{status="dead"} 0
forge_cluster_leader{role="scheduler",node_id="abc123"} 1
forge_cluster_heartbeat_lag_seconds{node_id="abc123"} 0.2
```

### Health Check

`/_health` returns 200 if node is active, 503 if draining or dead.
