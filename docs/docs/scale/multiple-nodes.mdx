...
Nodes missing heartbeats past the dead threshold are marked dead. The effective threshold is the greater of the configured `dead_threshold_secs` (default 15s) and 3x the current adaptive heartbeat interval:
...
-- $1 = max(dead_threshold_secs, 3 * current_adaptive_interval)
```
...
Rate limiting with `key = "user"` or `key = "ip"` is enforced per-node using an in-memory token bucket. In a multi-node deployment, a user can make up to `requests × node_count` per window. Only `key = "global"` uses a shared PostgreSQL counter and is truly cluster-wide.
