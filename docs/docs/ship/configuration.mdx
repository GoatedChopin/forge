...
| `debounce_quiet`              | string | `"50ms"` | Coalesce DB changes arriving within this window before triggering re-execution                |
| `debounce_max`                | string | `"200ms"`| Force a flush after this much time even if changes keep arriving                             |
...
| `listener_channel_buffer`     | usize  | `1024`   | Broadcast channel buffer for raw change notifications arriving from PostgreSQL                |
| `adaptive_row_threshold`      | usize  | `200`    | Switch from row-level to table-level change tracking per table above this subscription count  |
...
The debounce window matters for write-heavy workloads: `debounce_quiet` prevents re-execution storms when rows are updated in rapid succession, while `debounce_max` puts a ceiling on how long a client can wait for an update. The `adaptive_row_threshold` trades memory for precision — tables with fewer than that many subscriptions track individual row changes; above it, the engine tracks at table granularity (faster, slightly more false-positive invalidations).
