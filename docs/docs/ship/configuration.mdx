...
| `dns_name` | string | - | DNS name for discovery |

#### Discovery Methods

**postgres** (default)

Nodes register in a database table. No external service required. Works everywhere PostgreSQL works.
...
Query DNS for node addresses. Useful with service meshes.

```toml
[cluster]
discovery = "dns"
dns_name = "forge-nodes.default.svc.cluster.local"
```

**kubernetes**

Use Kubernetes API for pod discovery.
...
Hardcoded node list. Useful for testing or fixed infrastructure.

```toml
[cluster]
discovery = "static"
seed_nodes = ["10.0.1.1:9000", "10.0.1.2:9000"]
```
...
All discovery methods populate the same internal node registry:

```rust
pub enum DiscoveryMethod {
    Postgres,   // SELECT * FROM forge_nodes WHERE last_heartbeat > NOW() - INTERVAL '15s'
    Dns,        // DNS SRV/A record lookup
    Kubernetes, // Kubernetes API pod listing
    Static,     // Configured seed_nodes list
}
```

PostgreSQL discovery is the default because it requires no additional infrastructure. Nodes insert their address on startup, update on heartbeat, and get cleaned up when dead_threshold passes.
