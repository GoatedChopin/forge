...
```rust
// Simulate job lifecycle transitions in tests
ctx.job_dispatch().complete_job("send_email", json!({ "result": "sent" }));
ctx.job_dispatch().fail_job("send_email", "SMTP connection refused");
...
# Cargo.toml
[dev-dependencies]
forge = { version = "0.8.1", features = ["testcontainers"] }
```
...
```rust
let ctx = TestJobContext::builder("process_video")
    .with_saved("temp_file", json!("/tmp/video.mp4"))
    .build();
...
let ctx = TestJobContext::builder("long_job")
    ...
    .build();
assert!(ctx.check_cancelled().await.is_err()); // returns Err(JobCancelled)
```
...
// ACTION_TIMEOUT: 5s default for tokio::time::timeout in tests
// ASSERTION_TIMEOUT: for eventually-consistent assertion helpers
```
