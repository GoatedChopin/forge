...
Forge includes a built-in dashboard at `/_dashboard` for monitoring jobs, workflows, crons, and cluster state.

| Path | View |
|------|------|
| `/_dashboard` | Overview |
| `/_dashboard/jobs` | Job queue, status, dead letter |
| `/_dashboard/workflows` | Active and completed workflows |
| `/_dashboard/crons` | Scheduled tasks, history |
| `/_dashboard/cluster` | Node status, leader election |
| `/_dashboard/metrics` | Request metrics, latency |
| `/_dashboard/logs` | Structured logs |
| `/_dashboard/traces` | Distributed traces |

The dashboard API is available at `/_api/*` for custom integrations.
...
- Accurate cluster state in dashboard
...
- [ ] Monitoring for `/_api/metrics` endpoints
