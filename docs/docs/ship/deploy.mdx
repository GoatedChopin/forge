...
Enable direct TLS on the gateway (`[gateway.tls] enabled = true`) when:

- You need encrypted traffic between the load balancer and the app (zero-trust networks, ALB backend HTTPS, compliance requirements).
...
Forge supports two TLS modes:

### Self-signed mode

Generates a fresh certificate at process startup. No files to provision, no rotation, no cert-manager. Intended for the ALB-to-app encryption use case — the load balancer doesn't validate the backend's certificate chain, so an ephemeral self-signed cert is sufficient and eliminates operational overhead.
...
The certificate is regenerated on every restart. A new cert on each rollout is fine in this setup because it's never trusted by clients directly.

Startup logs a `WARN` line when self-signed mode is active:

```
Gateway listening with ephemeral self-signed TLS certificate. Not suitable for public-facing use; terminate public TLS at a load balancer.
```

### File-based mode

Load a real certificate and key from disk. Use this for production with a certificate issued by a CA (cert-manager, ACM Private CA, mounted Kubernetes secret, etc.).
...
- [ ] TLS configured (edge load balancer, or `[gateway.tls]` for zero-trust / ALB-backend HTTPS)
- [ ] If using `[gateway.tls]` with `cert_path`/`key_path`: file permissions locked down (e.g. `0400`, owned by the app user) and certificate rotation plan in place
- [ ] If using `[gateway.tls]` self-signed mode: load balancer health checks configured for HTTPS with no chain validation
