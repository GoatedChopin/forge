...
Private functions with input args must include an identity scope field like `user_id`, which the router validates against the JWT before your handler runs. Use `input.user_id` directly in your queries. For context-only handlers (no input struct), use `ctx.require_user_id()` to get the caller's UUID.
...
- **Identity scope is enforced automatically.** Private functions with input args must include an identity field (`user_id`, `owner_id`, `subject`, or `principal_id`), which is validated against the JWT before your handler runs. Use `input.user_id` directly in your SQL. Don't manually compare it against `ctx.require_user_id()`. For context-only handlers (no input struct), use `ctx.require_user_id()` to get the caller's UUID.
