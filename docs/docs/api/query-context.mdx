...
Access environment variables via the `EnvAccess` trait methods:

```rust
#[forge::query]
pub async fn get_settings(ctx: &QueryContext) -> Result<Settings> {
    ...
    // Get required variable (returns error if missing)
...
See [Environment Variables](/docs/api/env-access) for full API and testing.

## See Also

- [MutationContext](/docs/api/mutation-context) - For data modification
- [Environment Variables](/docs/api/env-access) - Typesafe env access
- [Functions](/docs/concepts/functions) - Function types overview
