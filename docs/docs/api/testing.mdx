...
FORGE provides a comprehensive testing framework for unit and integration testing your applications. This includes test contexts for all function types (queries, mutations, jobs, crons, workflows), zero-config database provisioning, HTTP mocking, and assertion macros for common test patterns.
...
```rust
// Option 1: Embedded PostgreSQL (recommended - zero config)
let db = TestDatabase::embedded().await?;

// Option 2: Explicit URL
let db = TestDatabase::from_url("postgres://localhost/test_db").await?;

// Option 3: From TEST_DATABASE_URL environment variable
...
```bash
# Tests use embedded PostgreSQL automatically
cargo test
...
| `env_require(key)` | Get required env var or error |
| `env_mock()` | Access mock provider for verification |
...
**HTTP Mocking (for mutations with external calls):**
...
All test contexts support mocking environment variables via `with_env()` and `with_envs()` builder methods.
...
vars.insert("TIMEOUT".to_string(), "30".to_string());
...
The mock provider tracks which variables were accessed:
...
// Call your function that uses env vars
...
assert!(ctx.env_mock().was_accessed("REQUIRED_KEY"));

// Get all accessed keys
let keys = ctx.env_mock().accessed_keys();
```
...
```rust
#[tokio::test]
async fn test_missing_env_fails() {
    let ctx = TestMutationContext::builder().build();  // No env vars
...
async fn test_env_parsing() {
    ...
    // Failed parse
    let result: Result<u32> = ctx.env_parse("INVALID");
    assert!(result.is_err());
}
```
...
- [Background Jobs](/docs/background/jobs) - Full jobs guide
- [Workflows](/docs/background/workflows) - Workflows guide
- [ForgeError](/docs/api/forge-error) - Error types
