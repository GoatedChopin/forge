...
Runs Docker Compose with PostgreSQL, backend (cargo-watch), and the selected frontend target. Use `forge dev down --clear` to reset. Pass `--target dioxus` to scaffold a Dioxus frontend instead of the default SvelteKit app.
