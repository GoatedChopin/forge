...
By default, jobs require authentication to dispatch. You can control access with these attributes:

### Public Jobs

Allow anyone to dispatch the job (no authentication required):

```rust
#[forge::job]
#[public]
pub async fn process_public_data(ctx: &JobContext, input: DataInput) -> Result<()> {
    // Can be dispatched without authentication
    Ok(())
}
```
...
Require a specific role to dispatch the job:

```rust
#[forge::job]
#[require_role("admin")]
pub async fn admin_cleanup(ctx: &JobContext, input: CleanupInput) -> Result<()> {
    // Only users with "admin" role can dispatch this job
    Ok(())
}
```
...
Without any auth attributes, jobs require authentication but no specific role:

```rust
#[forge::job]
pub async fn user_export(ctx: &JobContext, input: ExportInput) -> Result<()> {
    // Requires authentication, any authenticated user can dispatch
    Ok(())
}
```
...
```rust
#[forge::mutation]
pub async fn webhook_handler(ctx: &MutationContext, payload: WebhookPayload) -> Result<()> {
