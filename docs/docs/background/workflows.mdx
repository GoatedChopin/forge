...
| `public` | `#[public]` | Allow unauthenticated dispatch |
| `require_role` | `#[require_role("admin")]` | Require specific role to dispatch |

## Authorization

By default, workflows require authentication to start. You can control access with these attributes:

### Public Workflows

Allow anyone to start the workflow (no authentication required):

```rust
#[forge::workflow]
#[version = 1]
#[public]
pub async fn public_onboarding(ctx: &WorkflowContext, input: OnboardInput) -> Result<()> {
    // Can be started without authentication
    Ok(())
}
```
...
Require a specific role to start the workflow:

```rust
#[forge::workflow]
#[version = 1]
#[require_role("admin")]
pub async fn admin_bulk_operation(ctx: &WorkflowContext, input: BulkInput) -> Result<()> {
    // Only users with "admin" role can start this workflow
    Ok(())
}
```
...
Without any auth attributes, workflows require authentication but no specific role:

```rust
#[forge::workflow]
#[version = 1]
pub async fn user_verification(ctx: &WorkflowContext, input: VerifyInput) -> Result<()> {
    // Requires authentication, any authenticated user can start
    Ok(())
}
```
...
```rust
#[forge::mutation]
pub async fn approve_request(ctx: &MutationContext, input: ApproveInput) -> Result<()> {
