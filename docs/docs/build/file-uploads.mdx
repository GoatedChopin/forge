...
**Multipart processing**: The gateway reads multipart fields in chunks using `axum::extract::Multipart` while enforcing per-file, total-payload, and field-count limits. Accepted uploads are buffered in memory up to those limits.

**Size limits**: The gateway enforces a 10MB per-file limit, 20MB total payload limit, and 20 maximum upload fields. Multipart requests are also capped at 32 concurrent in-flight uploads to prevent memory exhaustion.

```rust
const MAX_FILE_SIZE: usize = 10 * 1024 * 1024;       // 10MB per file
const MAX_TOTAL_UPLOAD_SIZE: usize = 20 * 1024 * 1024; // 20MB total payload
