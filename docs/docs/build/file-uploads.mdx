...
The gateway resolves the effective limit in priority order: per-mutation `max_size` > global `max_body_size` > 20 MB default floor. The field-count limit (20 upload fields) and concurrency cap (32 concurrent in-flight uploads) remain fixed.
