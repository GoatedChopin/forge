...
function uploadWithProgress(
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.open("POST", `${API_URL}/_api/rpc/upload_image/upload`);
...
**Multipart processing**: The gateway reads multipart fields in chunks using `axum::extract::Multipart` while enforcing total-payload and field-count limits. Accepted uploads are buffered in memory up to those limits.

**Size limits**: Upload size limits are configurable at two levels. The global default is set in `forge.toml`:

```toml
[gateway]
max_body_size = "100mb"   # default: "20mb"
```

Individual mutations can override the global limit with the `max_size` attribute:

```rust
#[forge::mutation(max_size = "200mb")]
async fn upload_large_file(ctx: &Ctx, file: Upload) -> Result<String> {
    // This mutation accepts uploads up to 200 MB,
    // regardless of the global max_body_size setting.
}
```

The gateway resolves the effective limit in priority order: per-mutation `max_size` > global `max_body_size` (default 20 MB). The field-count limit (20 upload fields) and concurrency cap (32 concurrent in-flight uploads) remain fixed.
...
if (hasFiles) {
  ...
  // POST to /_api/rpc/{function}/upload with multipart
} else {
  // POST to /_api/rpc/{function} with JSON
}
```
...
async fn test_upload_avatar() {
    let ctx = TestMutationContext::builder()
        ...
        .mock_http("storage.example.com/*", |_req| {
            MockResponse::json(serde_json::json!("https://storage.example.com/avatar.png"))
        })
        .build();
