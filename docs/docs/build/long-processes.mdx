...
| `active` | flag | `true` | This version handles new runs. At most one active version per name. Default when neither flag is set. |
| `deprecated` | flag | `false` | Keeps executing in-flight runs but rejects new starts. Cannot combine with `active`. |
...
#[forge::workflow(name = "user_signup", version = "v1", deprecated)]
...
#[forge::workflow(name = "user_signup", version = "v2", active)]
