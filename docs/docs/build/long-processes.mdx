...
The runtime defends step 4: at boot it scans `forge_workflow_runs` for non-terminal runs whose `(name, version)` tuple is no longer in the registry. If any are found, it logs a warning per stranded group and flips `/_api/ready` to `503` with `drain_pending > 0`. Boot still succeeds, so the node stays up but the load balancer rotates traffic away.
