...
Mutations change your database state with built-in transactional guarantees.
...
Forge wraps the mutation in a database transaction. If the function returns `Ok`, changes commit. If it returns `Err` or panics, everything rolls back.
...
| `transactional` | flag | - | Explicit transaction with outbox pattern |
...
The HTTP client (`ctx.http()`) tracks failure rates per host. After repeated failures, it fails fast rather than waiting for timeouts. This prevents cascade failures when downstream services are unhealthy.
