...
|--------|-----------|-------------|
| `fetch_one` | `async fn fetch_one<O>(&self, query) -> Result<O>` | Fetch exactly one row |
| `fetch_optional` | `async fn fetch_optional<O>(&self, query) -> Result<Option<O>>` | Fetch zero or one row |
| `fetch_all` | `async fn fetch_all<O>(&self, query) -> Result<Vec<O>>` | Fetch all matching rows |
| `execute` | `async fn execute(&self, query) -> Result<PgQueryResult>` | Execute a statement |
...
pub async fn list_items(db: DbConn<'_>) -> Result<Vec<Item>> {
    db.fetch_all(sqlx::query_as("SELECT * FROM items ORDER BY created_at DESC")).await.map_err(Into::into)
}
...
| Method | Signature | Description |
|--------|-----------|-------------|
| `db` | `fn db(&self) -> &PgPool` | Database connection pool (works directly with `query_as!` macros) |
...
| `http` | `fn http(&self) -> &Client` | HTTP client for external calls |
| `require_user_id` | `fn require_user_id(&self) -> Result<Uuid>` | Get user ID or return error |
| `require_subject` | `fn require_subject(&self) -> Result<&str>` | Get subject or return error |
| `dispatch_job` | `async fn dispatch_job<T: Serialize>(&self, job_type: &str, args: T) -> Result<Uuid>` | Dispatch a background job |
| `dispatch_job_with_context` | `async fn dispatch_job_with_context<T: Serialize>(&self, job_type: &str, args: T, context: Value) -> Result<Uuid>` | Dispatch job with initial context |
...
| `http_with_circuit_breaker` | `fn http_with_circuit_breaker(&self) -> &CircuitBreakerClient` | HTTP client with circuit breaker |
