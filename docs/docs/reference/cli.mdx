...
| `--no-pg` | Use external `DATABASE_URL` instead of embedded PostgreSQL |
| `--backend-port <port>` | Backend HTTP port (default: `8080`) |
| `--frontend-port <port>` | Frontend Vite port (default: `5173`) |
| `--db-port <port>` | Embedded PostgreSQL port (default: `5432`) |
| `--takeover-ports` | Kill process(es) using requested ports and take over |
...
By default, `forge dev` fails fast when requested ports are occupied and prints the process using each blocked port.
...
### Watch Scope (bare metal)

`forge dev` only watches backend-relevant files for restarts:
...
`frontend/*` and unrelated root files (for example `*.json`) no longer trigger backend restarts.
