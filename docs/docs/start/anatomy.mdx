...
└── frontend/            # SvelteKit app (optional)
    ...
    │           ├── datetime.ts
    │           └── index.ts
    └── node_modules/    # Dependencies (includes @forge-rs/svelte)
```
...
The `@forge-rs/svelte` npm package provides the runtime library that powers subscriptions and API calls.
