...
## Zero-Dependency Deployment (Embedded PostgreSQL)

For small projects, demos, or self-hosted deployments, FORGE can run with **embedded PostgreSQL** - no external database needed.
...
That's it. On first start, FORGE downloads and runs a bundled PostgreSQL instance automatically.

### When to Use Embedded PostgreSQL

**Good for:**
- Personal projects and demos
- Small team applications (under 100 concurrent users)
- Self-hosted single-server deployments
- Offline/air-gapped environments
- Development without Docker

**Not recommended for:**
- High-availability production (no replication)
- Large-scale applications (limited to single machine)
- Environments requiring managed backups

### Data Location

Embedded PostgreSQL stores data in the working directory:
...
To backup: copy the `.forge/postgres/` directory.

### Combining with Embedded Frontend

For a truly zero-dependency deployment (single binary, no external services):
...
```bash
# Build with embedded frontend (default)
...
|----------|-------------|
| `DATABASE_URL` | PostgreSQL connection string (required unless `database.embedded = true`) |
