...
export class ForgeClient {
  ...
  async connect(): Promise<void> {
    ...
    const sseUrl = `${this.config.url}/events${params.toString() ? `?${params}` : ""}`;
  ...
  async call<T>(functionName: string, args: unknown): Promise<T> {
    ...
    if (hasFiles) {
      ...
      response = await fetch(`${this.config.url}/rpc/${functionName}/upload`, {
        method: "POST",
      ...
      response = await fetch(`${this.config.url}/rpc/${functionName}`, {
        method: "POST",
        headers: {
  ...
  private async unregisterSseSubscription(id: string): Promise<void> {
    const token = await this.getToken();
    await fetch(`${this.config.url}/unsubscribe`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
  ...
  async _registerJob(clientSubId: string, jobId: string): Promise<void> {
    ...
    const token = await this.getToken();
    await fetch(`${this.config.url}/subscribe-job`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
  ...
  async _registerWorkflow(clientSubId: string, workflowId: string): Promise<void> {
    if (!this.sessionId) return;
    const token = await this.getToken();
    await fetch(`${this.config.url}/subscribe-workflow`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
