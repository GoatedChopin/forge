...
// for the Dioxus SDK. Notable differences:
//   - vital(name, value, rating) batches as a `webvital.<name>` event posted to
//     /_api/signal/event (no /vital endpoint, no attribution propagated)
//   - capture_error always wires stack: None on the wire; the test bridge folds
//     Error.stack into context.stack so we still assert on it via context
//   - window.onerror handler in the SDK only forwards the message string
//     (web_sys ErrorEvent.message), so auto-captured stack is empty
...
test.describe("signals: client SDK via browser", () => {
  ...
  test("view: auto page view carries UTMs from initial URL", async ({
    page,
    gotoReady,
  }) => {
    // The Dioxus SDK extracts UTMs from window.location at init and stitches
    // them into the FIRST page() call (auto-fired ~2s after mount). It does
    // not parse UTMs from the path string, so we have to navigate with the
    // query already attached.
  ...
  test("report: captureError with stack, context, breadcrumbs, page_url", async ({
    ...
    // The Dioxus SDK never sets the wire-level `stack` field. The test bridge
    // folds Error.stack into context.stack so the spec can still assert on
    // both — matches what end users get when they look at the report payload.
    ...
    expect(err.context?.stack).toBeTruthy();
  ...
  test("report: unhandled errors are auto-captured", async ({ page }) => {
    ...
    // Dioxus SDK's window.onerror handler only forwards the message string
    // (web_sys::ErrorEvent.message), so stack is intentionally not asserted.
    expect(err.message).toContain("signals-spec-auto");
  });

  test("vital: manual vital() batches as webvital.<name> event", async ({
    page,
  }) => {
    // Dioxus SDK has no /vital endpoint. vital() funnels through track() with
    // the event name `webvital.<name>` and {value, rating} as properties.
    const eventPromise = waitForSignal(page, "event", (b) =>
      b.events.some((e) => e.event === "webvital.signals-spec-metric"),
    );
    await page.evaluate(() => {
    ...
    const { body } = await eventPromise;
    const evt = body.events.find(
      (e) => e.event === "webvital.signals-spec-metric",
    )!;
    expect(evt.properties).toMatchObject({ value: 123.45, rating: "good" });
  });
