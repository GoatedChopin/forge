...
// WASM SSE subscriptions don't always push updates to the DOM immediately.
// For mutations that change server state, we wait briefly then reload to
// guarantee the assertion sees fresh data.
async function waitAndReload(page: import("@playwright/test").Page) {
...
test.describe("Forge Demo", () => {
  ...
  // ---------------------------------------------------------------------------
  // Users CRUD (all via UI, with WASM reload workaround)
  // ---------------------------------------------------------------------------

  test("creating a user appears in the list", async ({ page, gotoReady }) => {
    await gotoReady();
