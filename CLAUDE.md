...
`emit.rs`: Single source of truth for ALL type mapping. `ts_type()` and `dioxus_type()` functions. Rust->TS: String/Uuid/DateTime->"string", i32/i64/f64->"number", bool->"boolean", Option<T>->"T|null", Vec<T>->"T[]", Upload->"File|Blob".
